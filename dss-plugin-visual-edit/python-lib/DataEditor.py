...
class DataEditor:
    ...
    def __append_to_editlog__(
        ...
        # turn value into a string (which is used as storage type) unless it's None
    ...
    def validate_row(self, row: dict) -> EditSuccess | EditFailure:
        """
        Validates a row.

        Args:
            row (dict): A dictionary containing all column values of the row to validate. This includes primary key(s), which serve to identify the row in the editlog; all the rest is the "context".

        Returns:
            EditSuccess | EditFailure: An object indicating the success or failure to insert an editlog.

        Notes:
            - Importance of logging the context: the context is the row as it was at the time of validation. Some of the row's values could have been generated by an upstream data flow, which could change in time. Logging the context is a way to log what exactly was validated. If this wasn't done, the entry of the edits dataset for the given primary key values would only contain the action "validate", which would be of little use.
    ...
    def invalidate_row(self, row: dict) -> EditSuccess | EditFailure:
        ...
        Args:
            row (dict): A dictionary containing all column values of the row to invalidate. This includes primary key(s), which serve to identify the row in the editlog; all the rest is the "context".

        Returns:
            EditSuccess | EditFailure: An object indicating the success or failure to insert an editlog.

        Notes:
            - This method is only used when a row was validated by error. Therefore, an 'invalidate' action would always follow a 'validate' action in the editlog. The context doesn't need to be logged since it will be the same as the one logged in the previous action (which could also be an 'update').
            - Attribution of the 'invalidate' action in the editlog: the user identifier is only logged when this method is called in the context of a webapp served by Dataiku (which allows retrieving the identifier from the HTTP request headers sent by the user's web browser).
        """
        primary_keys_tuple = get_key_values_as_tuple(row, self.primary_key_column_names)
        return self.__append_to_editlog__(
            primary_keys_tuple=primary_keys_tuple,
