...
def add_edit(cell):
    ...
    If the cell is in the Reviewed column, we (in)validate the row according to the value (True/False).
    """

    row = cell["row"]
    column_name = cell["field"]
    value = cell["value"]
    if column_name.lower() == "reviewed":
        if value.lower() == "true":
...
def create_endpoint():
    ...
    de.create_row(primary_keys, column_values)
