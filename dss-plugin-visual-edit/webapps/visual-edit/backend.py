...
def add_edit(cell):
    ...
    row_dic = cell["row"]  # this contains all column values, including the primary keys
    updated_field = cell["field"]
    updated_value = cell["value"]

    if updated_field == de.validation_column_name and updated_value.lower() == "true":
        status = de.validate_row(row_dic)
    elif updated_field == de.notes_column_name:
        status = de.comment_row(row_dic, updated_value)
    else:
        status = de.update_row(row_dic, updated_field, updated_value)

    info = ""
    for s in status:
        info += __edit_result_to_message__(s) + "\n"
...
def update_endpoint():
    ...
    status = de.update_row(primary_keys_values, column, value)
    response = jsonify({"msg": __edit_result_to_message__(status)})
    return response
